...
**Pitfall — Chinese manuscript length:** The `check_section_lengths.py` script is calibrated for English manuscripts (6,000-9,000 word budgets) and cannot reliably detect Chinese section headers or character counts. For a standard Chinese ecology research article, target 12,000-16,000 Chinese characters (汉字) in the main text, with Introduction ~1,500-2,500 chars, Discussion ~3,000-5,000 chars, and Conclusion ~500-800 chars. Unless the journal contract or article type explicitly requires a shorter form, a Chinese manuscript below 12,000 characters is under target; do not pad it mechanically—identify and complete the missing scientific or writing functions.
