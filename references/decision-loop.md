...
## Gate 2: Answerable Question（步骤 13 之后）— 🟡 ADP-1 作者决策点（默认硬停）

> 这是作者决策点：摆出探索分析摘要 + 2-4 个候选方向后由作者拍板（详见 `author-decision-points.md`）。默认 `interactive` 模式下，作者必须选择、调整或降级后才能继续。只有用户或运行配置明确要求 `unattended` 自动运行时，作者无响应才按最保守的推荐默认方向继续。

| Decision | Condition | Action |
|----------|-----------|--------|
| **PROCEED** | 存在具体的、文献未充分回答的、数据可回答的问题，且作者已选择方向（或显式 `unattended` 模式采用保守默认） | 继续 → 构建 argument contract |
...
- **ADP（作者决策点）**：Gate 2 / Gate 3 以及 claim ledger（步骤 21，ADP-3）是三个作者决策点。
- **ADP-1 / Gate 2**：默认是 `interactive` 硬停，必须由作者确认方向；只有显式 `unattended` 自动运行时，才可按最保守默认继续。
- **ADP-2 / Gate 3 与 ADP-3 / claim ledger**：软停带保守默认，摆出证据后可继续。详见 `author-decision-points.md`。
